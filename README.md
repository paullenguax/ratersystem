...
- **Finding the right test to link**: the "New session" dialog's test picker is filterable by `Test.courseTag` (`rater_course`/`refresher_course`/`other`) and sorted by `Test.dayLabel` (a free-text field like "Day 1", plain string-sorted) then test number — both fields are optional, editable in Test Bank's drawer, and unrelated to `category` (which separates the rater-course pool from the standardization one; `courseTag` is a sub-classification within it, for a trainer picking a specific test quickly rather than for pooling/isolation). The picker itself only ever offers rater-course-category tests — standardization tests are reserved for real interlocutor calibration assignments, not previewed here.
