...
export function OfficialFormsPage() {
  ...
  async function handleCaaGenerate(e: React.FormEvent) {
    ...
    try {
      ...
      if (msStatus === 'connected') {
        try {
          spUrl = (await uploadToSharePoint(blob, filename, SP_FOLDER_CAA, 'SUPERADMIN')).webUrl
  ...
  async function handleDgacGenerate(e: React.FormEvent) {
    ...
    try {
      ...
      if (msStatus === 'connected') {
        try {
          spUrl = (await uploadToSharePoint(blob, filename, SP_FOLDER_DGAC, 'SUPERADMIN')).webUrl
